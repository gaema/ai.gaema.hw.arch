...
export function dualDieMap() {
  ...
  return {
    title: "One p300c card — both ASICs and the PCB between them",
    cols: 17, rows: 12 + BAND + 12, cell: 54, cellH: 34,
    tiles,
    // The board outline. The connectors are INSIDE it -- they are soldered to
    // this PCB. What is NOT on the board is the cable that mates with them, and
    // that only appears on the second map, which is where the thing on the other
    // end of it lives.
    groups: [{ x0: 0, y0: 0, x1: 16, y1: LOWER + 11, label: "One p300c card — one PCB" }],
...
export function quietBoxMap() {
  ...
  // an arc into the earlier one would vanish. The connector is ONE column at the
  // card's own edge rather than a block parked beside it -- it is a connector,
  // not a third major component, and giving it its own wide block both oversells
  // it and pushes the two boards apart for nothing.
  //   0..6 card A dies   7 card A edge   8 gap
  //   9 the cable
  //   10 gap             11 card B edge  12..18 card B dies
  const CARD_W = 7;
  const A = { x: 0, warp: 7 };
  const CABLE = 9;
  const B = { x: 12, warp: 11 };
  // THE OFFSET, and it is the whole reason the board has two positions. Both
  // cards are the same part; each populates the position its twin does not, so
  // the fitted connectors end up diagonally opposite and the cable runs at an
  // angle rather than straight across. Card A takes the upper position here,
  // card B the lower -- A and B are labels for this drawing, and the point is
  // only that they differ.
  const FITTED_Y = { A: 0, B: 4 }, EMPTY_Y = { A: 4, B: 0 };
  ...
  const arcs = [
    ...
    { from: [A.warp, FITTED_Y.A + 1], to: [CABLE, 3], color: io, dip: 0.15,
      label: "Card A's connector → the ARP6 cable, leaving the board from the UPPER position" },
    { from: [CABLE, 3], to: [B.warp, FITTED_Y.B + 1], color: io, dip: 0.15,
      label: "The cable → card B's connector, arriving at the LOWER position — the offset is why it runs diagonally" },
  ];
  ...
  return {
    ...
    interconnect: "Three kinds of connection, and the whole point of the map is that they are not the same. INSIDE a die, the NOC mesh — not drawn here; the map above draws it. BETWEEN the two dies OF ONE CARD, two Ethernet links routed as differential pairs on that card's own PCB, with no connector anywhere on the path. BETWEEN THE TWO CARDS, a Samtec ARP6 cable — the only hop here that leaves a PCB, and the reason the two cards are drawn as two outlines rather than one. Nothing is shared across it: no NOC, no memory, no address space. Four dies in a box are four devices that talk.",
    note: "Board scale, so a die is one block: the 17 × 12 NOC grid inside each of these is the map above. THE TWO CARDS ARE THE SAME PART, which is what the two Warp 400 positions are for — each card populates the one its twin leaves empty, so the fitted connectors are offset and the cable crosses diagonally instead of running straight across. A and B are labels for this drawing; which physical position a given card populates is a property of how it is built for its place in the box, and tt-metal's board definition declares both positions without distinguishing them, so software describes a board with two where any one card has one.",
